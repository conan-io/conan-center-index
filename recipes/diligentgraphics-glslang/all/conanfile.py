...
class GlslangConan(ConanFile):
    name = "diligentgraphics-glslang"
    description = "Diligent fork. Khronos-reference front end for GLSL/ESSL, partial front " \
                  "end for HLSL, and a SPIR-V generator."
    license = ["BSD-3-Clause", "NVIDIA"]
    topics = ("conan", "glslang", "glsl", "hlsl", "spirv", "spir-v", "validation", "translation", "diligent")
    ...
    def _get_compatible_spirv_tools_version(self):
        return {
            "cci.20211008": "cci.20211008",
        }.get(str(self.version), False)

    def requirements(self):
        if self.options.enable_optimizer:
            self.requires("diligentgraphics-spirv-tools/{}".format(self._get_compatible_spirv_tools_version))

    def validate(self):
        if self.options.enable_optimizer and self.options["diligentgraphics-spirv-tools"].shared:
    ...
    def package_info(self):
        ...
        if self.options.enable_optimizer:
            self.cpp_info.components["spirv"].requires.append("diligentgraphics-spirv-tools::spirv-tools-opt")
