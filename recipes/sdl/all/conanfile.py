    ...
    def _configure_cmake(self):
        if not self._cmake:
            ...
            if self.settings.os == "Linux":
                ...
                if self.options.alsa:
                    self._cmake.definitions["ALSA_SHARED"] = self.options["libalsa"].shared
                ...
                if self.options.jack:
                    self._cmake.definitions["JACK_SHARED"] = self.options["jack"].shared
                self._cmake.definitions["ESD"] = self.options.esd
                if self.options.esd:
                    self._cmake.definitions["ESD_SHARED"] = self.options["esd"].shared
                self._cmake.definitions["PULSEAUDIO"] = self.options.pulse
                if self.options.pulse:
                    self._cmake.definitions["PULSEAUDIO_SHARED"] = self.options["pulseaudio"].shared
                self._cmake.definitions["SNDIO"] = self.options.sndio
                if self.options.sndio:
                    self._cmake.definitions["SNDIO_SHARED"] = self.options["sndio"].shared
