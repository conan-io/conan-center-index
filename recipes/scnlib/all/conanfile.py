...
class ScnlibConan(ConanFile):
    ...
    def _compilers_minimum_version(self):
        return {
            "gcc": "5",
            "clang": "6.0",
            "Visual Studio": "16" if tools.scm.Version(self.version) >= "1.0" else "15",
        }

    def requirements(self):
        if tools.scm.Version(self.version) >= "1.0":
    ...
    def validate(self):
        ...
        if minimum_version and tools.scm.Version(self.settings.compiler.version) < minimum_version:
    ...
    def package(self):
        ...
        if tools.scm.Version(self.version) >= "1.0":
            tools.files.rm(self, "*.cmake", os.path.join(self.package_folder, "include", "scn", "detail"))
