...
class ICUBase(ConanFile):
    ...
    def original_version(self):
        ...
        return '.'.join(original_version[:2])
    ...
    def source(self):
        tools.get(**self.conan_data["sources"][self.original_version], strip_root=True, destination=self._source_subfolder)
    ...
    def _install_name_tool(self):
        if tools.is_apple_os(self.settings.os):
            with tools.chdir(os.path.join(self.package_folder, "lib")):
                for dylib in glob.glob("*icu*.{0}.dylib".format(self.original_version)):
    ...
    def _data_path(self):
        ...
        if self.settings.os == "Windows" and self.settings.build_type == "Debug":
            data_dir_name += "d"
        data_dir = os.path.join(self.package_folder, "lib", data_dir_name, self.original_version)
        return os.path.join(data_dir, self._data_filename)

    @property
    def _data_filename(self):
        vtag = self.original_version.split(".")[0]
