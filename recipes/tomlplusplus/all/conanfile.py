...
class TomlPlusPlusConan(ConanFile):
    ...
    def _minimum_compilers_version(self):
        return {
            "Visual Studio": "16" if tools.scm.Version(self.version) < "2.2.0" or tools.scm.Version(self.version) >= "3.0.0" else "15",
            "gcc": "7",
            "clang": "5",
            "apple-clang": "10",
        }
    ...
    def validate(self):
        ...
        if self.settings.compiler == "apple-clang" and tools.scm.Version(self.version) < "2.3.0":
            raise ConanInvalidConfiguration("The current compiler {} {} is supported in version >= 2.3.0".format(
                    self.settings.compiler, self.settings.compiler.version))

        if is_msvc(self) and tools.scm.Version(self.version) == "2.1.0":
