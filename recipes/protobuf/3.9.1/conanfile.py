...
class ProtobufConan(ConanFile):
    ...
    def _can_disable_rtti(self):
        return tools.Version(self.original_version) >= "3.15.4"
    ...
    def configure(self):
        ...
        if self.settings.compiler == "clang":
           if tools.Version(self.original_version) >= "3.15.4" and tools.Version(self.settings.compiler.version) < "4":
                raise ConanInvalidConfiguration("protobuf {} doesn't support clang < 4".format(self.original_version))
    ...
    def source(self):
        ...
        extracted_folder = self.name + "-" + self.original_version
    ...
    def _configure_cmake(self):
        if not self._cmake:
            ...
            if tools.Version(self.original_version) >= "3.14.0":
