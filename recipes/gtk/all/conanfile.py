...
class GtkConan(ConanFile):
    ...
    @property
    def _gtk4(self):
        return tools.scm.Version("4.0.0") <= tools.scm.Version(self.version) < tools.scm.Version("5.0.0")

    @property
    def _gtk3(self):
        return tools.scm.Version("3.0.0") <= tools.scm.Version(self.version) < tools.scm.Version("4.0.0")
    ...
    def validate(self):
        ...
        if str(self.settings.compiler) in ["Visual Studio", "msvc"]:
            if tools.scm.Version(self.version) < "4.2":
    ...
    def _configure_meson(self):
        ...
        if self._gtk4:
            ...
            if tools.scm.Version(self.version) < "4.3.2":
    ...
    def build(self):
        ...
        if "4.2.0" <= tools.scm.Version(self.version) < "4.6.1":
            tools.files.replace_in_file(self, os.path.join(self._source_subfolder, "meson.build"),
                                  "gtk_update_icon_cache: true",
                                  "gtk_update_icon_cache: false")
        if "4.6.2" <= tools.scm.Version(self.version):
            tools.files.replace_in_file(self, os.path.join(self._source_subfolder, "meson.build"),
