...
class OpenCVConan(ConanFile):
  ...
  set(Protobuf_INCLUDE_DIR ${protobuf_INCLUDE_DIR})  # patch: map conan generated v ariable''')
    ...
    def _configure_cmake(self):
        ...
        if self.options.dnn:
            self._cmake.definitions["PROTOBUF_UPDATE_FILES"] = True  # force rebuild of proto files
        if self.options.contrib:
    ...
    def package_info(self):
        ...
        def protobuf():
            return ["protobuf::libprotobuf"] if self.options.dnn else []
        ...
        if self.options.dnn:
            add_components([
                {"target": "opencv_dnn", "lib": "dnn", "requires": ["opencv_core", "opencv_imgproc"] + protobuf()},  # TODO find other dependencies, how?
            ])
