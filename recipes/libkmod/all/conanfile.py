...
class LibKMODConan(ConanFile):
    name = "libkmod"
    version = "system"
    description = "Linux kernel module handling library"
    topics = ("kmod", "kernel", "modules")
    ...
    def validate(self):
        if self.settings.os != "Linux":
            raise ConanInvalidConfiguration("libkmod is only supported on Linux.")
    ...
    def _fill_cppinfo_from_pkgconfig(self, name):
        ...
        if not pkg_config.provides:
            raise ConanException("libkmod development files aren't available")
    ...
    def package_info(self):
        ...
        self._fill_cppinfo_from_pkgconfig("libkmod")
