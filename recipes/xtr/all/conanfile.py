...
class XtrConan(ConanFile):
    ...
    def config_options(self):
        if tools.scm.Version(self.version) < "1.0.1":
    ...
    def requirements(self):
        ...
        if tools.scm.Version(self.version) >= "2.0.0" and self.settings.os == "Linux" and self.options.get_safe("enable_io_uring"):
    ...
    def validate(self):
        ...
        if tools.scm.Version(self.version) < "2.0.0" and self.settings.compiler == "clang" and self.settings.compiler.libcxx == "libc++":
    ...
    def build(self):
        ...
        if tools.scm.Version(self.deps_cpp_info["fmt"].version) < 6:
            raise ConanInvalidConfiguration("The version of fmt must >= 6.0.0")
        if tools.scm.Version(self.deps_cpp_info["fmt"].version) == "8.0.0" and self.settings.compiler == "clang":
