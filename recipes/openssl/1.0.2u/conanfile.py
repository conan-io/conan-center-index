...
class OpenSSLConan(ConanFile):
    ...
    def build(self):
        ...
        with self._make_context():
            with chdir(self, self.source_folder):
                ...
                if self._use_nmake:
                    do_bat = "ms/do_nasm" if self.settings.arch == "x86" else "ms/do_win64a"
                    self.run(os.path.abspath(do_bat))
                    self._replace_runtime_in_file("ms/ntdll.mak")
                    self.run("nmake -f ms/ntdll.mak")
                else:
                    autotools.make()
    ...
    def package(self):
        ...
        if self._use_nmake:
            with chdir(self, self.source_folder):
                self.run(f"nmake -f ms/ntdll.mak install")
        else:
            autotools = Autotools(self)
            with chdir(self, self.source_folder):
    ...
    def package_info(self):
        ...
        if self.settings.os == "Windows":
            self.cpp_info.components["crypto"].system_libs.extend(["crypt32", "ws2_32", "advapi32", "user32", "gdi32"])
