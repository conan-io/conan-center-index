...
class DCMTKConan(ConanFile):
    ...
    def generate(self):
        ...
        if self.options.charset_conversion == "libiconv":
            tc.variables["WITH_LIBICONVINC"] = self.deps_cpp_info["libiconv"].rootpath.replace("\\", "/")
        ...
        if self.options.with_openssl:
            tc.variables["WITH_OPENSSLINC"] = self.deps_cpp_info["openssl"].rootpath.replace("\\", "/")
        tc.variables["DCMTK_WITH_PNG"] = self.options.with_libpng
        if self.options.with_libpng:
            tc.variables["WITH_LIBPNGINC"] = self.deps_cpp_info["libpng"].rootpath.replace("\\", "/")
        ...
        if is_msvc(self):
            ...
            # tc.variables["HAVE_SSL_CTX_GET0_PARAM"] = 1
    ...
    def package(self):
        copy(self, pattern="COPYRIGHT", src=self.source_folder, dst=os.path.join(self.package_folder, "licenses"))
