...
class PulseAudioConan(ConanFile):
    ...
    def generate(self):
        ...
        for lib in ["alsa", "x11", "openssl", "dbus"]:
            tc.project_options[lib] = yes_no(getattr(self.options, f'with_{lib}'))

        # TODO: Default is tdb but the package is not available in conan
        tc.project_options["database"] = "gdbm" if self.options.with_gdbm else "simple"
        tc.generate()
