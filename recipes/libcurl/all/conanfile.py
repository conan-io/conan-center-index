...
class LibcurlConan(ConanFile):
    ...
    default_options = {
        ...
        "with_libssh2": True,
        "with_libidn": True,
        # TODO: Check, compilation fails in True locally. This looks like a missing dependency looking at autotools!
        # Windows can't find it as part of openssl_check_symbol_exists as a openssl component
        # Autotools also complains with error: --librtmp was specified but could not find librtmp pkgconfig file.
        "with_librtmp": False,
        # This has not CCI recipe, but users might be finding it locally in the system,
        # so we can't validate it out.
        "with_libgsasl": False,
        "with_libpsl": True,
        "with_largemaxwritesize": True,
        "with_nghttp2": True,
        "with_zlib": True,
        "with_brotli": False,
        "with_zstd": False,
    ...
    def config_options(self):
        ...
        if self._is_using_cmake_build and Version(self.version) < "8.10.0":
    ...
    def _patch_cmake(self):
        ...
        if Version(self.version) < "8.10.0":
            replace_in_file(self, cmakelists, "${CARES_LIBRARY}", "${CARES_LIBRARIES}")
            replace_in_file(self, cmakelists, "${LIBPSL_LIBRARY}", "${LIBPSL_LIBRARIES}")
            replace_in_file(self, cmakelists, "${LIBSSH2_LIBRARY}", "${LIBSSH2_LIBRARIES}")
    ...
    def _generate_with_cmake(self):
        ...
        if self.options.with_zstd:
            ...
            tc.cache_variables["ZSTD_VERSION"] = str(self.dependencies["zstd"].ref.version)
        tc.variables["CURL_USE_LIBPSL"] = self.options.with_libpsl
        if self.options.with_libpsl:
            # Can't force CMakeDeps to generate this
            tc.cache_variables["LIBPSL_FOUND"] = True
        ...
        if self.options.with_c_ares:
            deps.set_property("c-ares", "cmake_file_name", "CARES" if  Version(self.version) < "8.10.0" else "Cares")
            deps.set_property("c-ares", "cmake_additional_variables_prefixes", ["CARES"])
        if self.options.get_safe("with_libpsl"):
            deps.set_property("libpsl", "cmake_file_name", "LibPSL")
            deps.set_property("libpsl", "cmake_additional_variables_prefixes", ["LIBPSL"])
        if self.options.with_libidn:
        ...
        if self.options.with_libssh2:
            ...
            deps.set_property("libssh2", "cmake_additional_variables_prefixes", ["LIBSSH2"])
        if self.options.with_ssl == "wolfssl":
            deps.set_property("wolfssl", "cmake_file_name", "WolfSSL")
            deps.set_property("wolfssl", "cmake_additional_variables_prefixes", ["WOLFSSL"])
        elif self.options.with_ssl == "mbedtls":
