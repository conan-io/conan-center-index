...
class LibcurlConan(ConanFile):
    ...
    def _has_zstd_option(self):
        return tools.scm.Version(self, self.version) >= "7.72.0"

    @property
    def _has_metalink_option(self):
        # Support for metalink was removed in version 7.78.0 https://github.com/curl/curl/pull/7176
        return tools.scm.Version(self, self.version) < "7.78.0" and not self._is_using_cmake_build
    ...
    def configure(self):
        ...
        if self._is_using_cmake_build:
            if tools.scm.Version(self, self.version) < "7.75.0":
    ...
    def _patch_cmake(self):
        ...
        if tools.scm.Version(self, self.version) >= "7.80.0":
            tools.files.replace_in_file(self, 
                cmakelists,
    ...
    def _configure_cmake(self):
        ...
        if tools.scm.Version(self, self.version) >= "7.81.0":
            cmake.definitions["CURL_USE_SCHANNEL"] = self.options.with_ssl == "schannel"
        elif tools.scm.Version(self, self.version) >= "7.72.0":
        ...
        elif tools.scm.Version(self, self.version) >= "7.70.0":
