...
class ConanSqlite3(ConanFile):
    ...
    def _has_enable_math_function_option(self):
        return tools.Version(self.original_version) >= "3.35.0"
    ...
    def source(self):
        tools.get(**self.conan_data["sources"][self.original_version], destination=self._source_subfolder, strip_root=True)

    def _configure_cmake(self):
        if self._cmake:
            return self._cmake
        self._cmake = CMake(self)
        self._cmake.definitions["SQLITE3_VERSION"] = self.original_version
