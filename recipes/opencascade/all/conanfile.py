...
class OpenCascadeConan(ConanFile):
    ...
    def validate(self):
        ...
        if (
            ...
            raise ConanInvalidConfiguration(
                f"{self.ref} doesn't support Clang 6.0 if Release build type"
            )
    ...
    def generate(self):
        ...
        # Since https://github.com/Open-Cascade-SAS/OCCT/commit/e92d322d4090d9611930bf7cabcc74fa981432b0
        if Version(self.version) > "7.6.2" and self.settings.compiler.get_safe(
            "cppstd"
        ):
            tc.variables[
                "BUILD_CPP_STANDARD"
            ] = "C++" + self.settings.compiler.get_safe("cppstd")

        tc.cache_variables["BUILD_LIBRARY_TYPE"] = (
        ...
        if self.settings.build_type == "Debug":
            tc.cache_variables[
                "BUILD_WITH_DEBUG"
            ] = self.options.extended_debug_messages
    ...
    def _patch_sources(self):
        ...
        sorted_deps = [
            dep for dep in reversed(self.dependencies.host.topological_sort.values())
        ]
        deps_defines = " ".join(
            [
                f"-D{d}"
                for dep in sorted_deps
                for d in dep.cpp_info.aggregated_components().defines
            ]
        )
        replace_in_file(
            self,
        ...
        for cmake_file in [cmakelists, cmakelists_tools]:
            deps_includedirs = ";".join(
                [
                    p.replace("\\", "/")
                    for dep in sorted_deps
                    for p in dep.cpp_info.aggregated_components().includedirs
                ]
            )
            replace_in_file(
                self,
            ...
            deps_libdirs = ";".join(
                [
                    p.replace("\\", "/")
                    for dep in sorted_deps
                    for p in dep.cpp_info.aggregated_components().libdirs
                ]
            )
            replace_in_file(
                self,
        ...
        freetype_libs = " ".join(
            self.dependencies["freetype"].cpp_info.aggregated_components().libs
        )
        replace_in_file(
            self,
            occt_csf_cmake,
        ...
        tcl_libs = " ".join(
            self.dependencies["tcl"].cpp_info.aggregated_components().libs
        )
        csf_tcl_libs = f'set (CSF_TclLibs "{tcl_libs}")'
        replace_in_file(
            self, occt_csf_cmake, 'set (CSF_TclLibs     "tcl86")', csf_tcl_libs
        )
        replace_in_file(self, occt_csf_cmake, "set (CSF_TclLibs   Tcl)", csf_tcl_libs)
        if Version(self.version) >= "7.6.0":
            replace_in_file(
                self, occt_csf_cmake, 'set (CSF_TclLibs   "tcl8.6")', csf_tcl_libs
            )
        else:
            replace_in_file(
                self, occt_csf_cmake, 'set (CSF_TclLibs     "tcl8.6")', csf_tcl_libs
        ...
        if self._link_tk:
            ...
            tk_libs = " ".join(
                self.dependencies["tk"].cpp_info.aggregated_components().libs
            )
        ...
        if self._is_linux:
            ...
            fontconfig_libs = " ".join(
                self.dependencies["fontconfig"].cpp_info.aggregated_components().libs
            )
            if Version(self.version) >= "7.6.0":
                replace_in_file(
                    self,
        ...
        if self.options.with_tbb:
            ...
            tbb_libs = " ".join(
                self.dependencies["onetbb"].cpp_info.aggregated_components().libs
            )
            replace_in_file(
                self,
                occt_csf_cmake,
        ...
        if self.options.with_ffmpeg:
            ...
            ffmpeg_libs = " ".join(
                self.dependencies["ffmpeg"].cpp_info.aggregated_components().libs
            )
            replace_in_file(
                self,
                occt_csf_cmake,
        ...
        if self.options.with_freeimage:
            ...
            freeimage_libs = " ".join(
                self.dependencies["freeimage"].cpp_info.aggregated_components().libs
            )
            replace_in_file(
                self,
                occt_csf_cmake,
        ...
        if self.options.with_openvr:
            ...
            openvr_libs = " ".join(
                self.dependencies["openvr"].cpp_info.aggregated_components().libs
            )
            replace_in_file(
                self,
                occt_csf_cmake,
        ...
        for module_line in modules_content.splitlines():
            ...
            for component_name in components_list:
                ...
                externlib_content = load(
                    self,
                    os.path.join(
                        self.source_folder, "src", component_name, "EXTERNLIB"
                    ),
                )
