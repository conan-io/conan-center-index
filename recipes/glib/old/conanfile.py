...
class GLibConan(ConanFile):
    ...
    def generate(self):
        ...
        tc.project_options["with-docs"] = "yes" if self.options.with_docs else "no"
        tc.project_options["with-man"] = "yes" if self.options.with_man else "no"
        tc.project_options["enable-dtrace"] = "true" if self.options.get_safe("enable_dtrace") else "false"
        tc.project_options["enable-libmount"] = "yes" if self.options.get_safe("with_mount") else "no"

        tc.generate()

        env = Environment()
        env.define_path("CL", "/utf-8")
        env.vars(self).save_script("conanbuild_msvc_options")
    
    def _patch_sources_intl(self):
         if self.settings.os != "Linux":
    ...
    def meson_build(self, target=None):
        """
        ...
        """
        meson_build_folder = self.build_folder
        ...
        if target:
            cmd += " {}".format(target)
        cmd += " --verbose"
        backend = self.conf.get("tools.meson.mesontoolchain:backend",
                                    default='ninja')
 
        xpPlatform = ",-p:WindowsTargetPlatformVersion=7.0" if self._is_xp() else ""
        toolset = msvs_toolset(self)
        if backend != "ninja":
            cmd += " --vs-args=-m,-p:PlatformToolset={}".format(toolset)
        cmd += xpPlatform
        self.output.info("Meson build cmd: {}".format(cmd))
        self.run(cmd)

    def _is_xp(self):
        toolset = msvs_toolset(self)
        return toolset.endswith("_xp")
