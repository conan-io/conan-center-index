...
class LLVMCoreConan(ConanFile):
    ...
    def _component_dependencies(self):
        """
        Returns a dictionary indexed by the LLVM component name (target) containing
    ...
    def _create_cmake_build_module(self, component_names, module_file):
        package_folder = Path(self.package_folder)
        content = textwrap.dedent(f"""\
            set(LLVM_PACKAGE_VERSION "{self.version}")
            set(LLVM_AVAILABLE_LIBS "{';'.join(component_names)}")
