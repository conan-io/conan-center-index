...
from conan.tools.files import apply_conandata_patches, collect_libs, get, rmdir, save, copy, export_conandata_patches, load, replace_in_file
...
class LLVMCoreConan(ConanFile):
        ...
        def _parse_deps(deps_list):
            ...
            for dep in deps_list.split(";"):
                ...
                if match:
                    ...
                    component = self._lib_name_to_component(dep)

                if component in ignore:
                    continue

                if component in ["rt", "m", "dl"]:
    ...
    def _create_cmake_build_module(self, components, module_file):
        ...
        json_text = json.dumps(components, indent=2).replace('"', '\\"')
        content = textwrap.dedent(f"""\
            ...
            function(llvm_map_components_to_libnames OUTPUT)
                ...
                foreach(_component ${{ARGN}})
                    string(JSON _lib_name ERROR_VARIABLE ERR GET ${{_components_json}} ${{_component}} "lib_name")
                    if (ERR)
