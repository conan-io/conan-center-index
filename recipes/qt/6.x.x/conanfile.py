...
class QtConan(ConanFile):
    ...
    def validate(self):
        if self.info.settings.compiler == "gcc" and Version(self.info.settings.compiler.version) == 11:
            raise ConanInvalidConfiguration("qt is not supported on gcc11 on C3I until conan-io/conan-center-index#13472 is fixed")
    ...
    def source(self):
        ...
        replace_in_file(self, os.path.join("qt6", "CMakeLists.txt"),
                        "enable_testing()",
        ...
        replace_in_file(self, os.path.join("qt6", "qtbase", "cmake", "QtInternalTargets.cmake"),
                              "-Zc:wchar_t",
    ...
    def _configure_cmake(self):
        ...
        try:
            ...
            if os.path.isfile(cmake_err_log):
                self.output.info(load(self, cmake_err_log))
            if os.path.isfile(cmake_out_log):
                self.output.info(load(self, cmake_out_log))
            raise
        return cmake
