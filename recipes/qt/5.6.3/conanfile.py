...
class QtConan(ConanFile):
    ...
    def validate(self):
        ...
        if "freetype" in self.dependencies:
            ...
            if freetype.options.with_bzip2 or freetype.options.with_brotli:
                raise ConanInvalidConfiguration("The freetype options with_bzip2 and with_brotli cannot be True.")
    ...
    def package_id(self):
        ...
        if self.info.settings.build_type == "MinSizeRel":
    ...
    def generate(self):
        ...
        if is_msvc(self):
            for libdir in [l for dependency in self.dependencies.host.values() for l in dependency.cpp_info.aggregated_components().libdirs]:
                env.prepend_path("LIB", libdir)
            for bindir in [l for dependency in self.dependencies.host.values() for l in dependency.cpp_info.aggregated_components().bindirs]:
    ...
    def get_dependence(self, names):
        for name in names:
            if name in self.dependencies:
                return self.dependencies[name].cpp_info
        return None

    def patch_dependency(self):
        libpng = self.get_dependence([])
        if libpng:
            libpng_pri = os.path.join(self.source_folder, "qt5/qtbase/src/3rdparty/png_dependency.pri")
            libpng_str = " ".join(["-l" + i for i in libpng.libs])
            save(self, libpng_pri, """contains(QT_CONFIG, system-png) {
    unix|mingw {
...
}""", " ".join(libpng.libs), libpng_str, libpng_str)

    def build(self):
        args = ["-confirm-license", "-nomake examples", "-no-warnings-are-errors", "-nomake tests",
                f"-prefix {self.package_folder}"]
        if self.options.target:
            args.append(f"-target {self.options.target}")
        ...
        if self.options.multiconfiguration:
            ...
            args.append("-release")
            self.output.warning("Only versions greater than 5.6.3 can use MinSizeRel,will fallback to Release.")
        ...
        if self.options.with_glib:
            args.append("-glib")
        args.append("-system-pcre" if self.options.with_pcre else "-qt-pcre")
        args.append("-fontconfig" if self.options.get_safe("with_fontconfig", False) else "-no-fontconfig")
        args.append("-icu" if self.options.get_safe("with_icu", False) else "-no-icu")
        args.append("-qt-sql-mysql" if self.options.get_safe("with_mysql", False) else "-no-sql-mysql")
        args.append("-qt-sql-psql" if self.options.with_pq else "-no-sql-psql")
        args.append("-qt-sql-odbc" if self.options.with_odbc else "-no-sql-odbc")

        if self.options.qtmultimedia and not self.is_windows:
            args.append("-alsa" if self.options.get_safe("with_libalsa", False) else "-no-alsa")
        ...
        if self.settings.os == "Macos":
            ...
            if self.settings.arch == "armv8":
                raise ConanInvalidConfiguration('unsupport QMAKE_APPLE_DEVICE_ARCHS="arm64"')
        elif self.settings.os == "Android":
        ...
        if not self.options.shared:
            ...
            if self.settings.os == "Macos" or (self.settings.os == "iOS" and Version(self.settings.compiler.version) >= "14.0"):
                self.cpp_info.components["qtCore"].frameworks.append(
                    "IOKit")  # qtcore requires "_IORegistryEntryCreateCFProperty", "_IOServiceGetMatchingService" and much more which are in "IOKit" framework
            if self.settings.os == "Macos":
