...
class QtConan(ConanFile):
    ...
    version="5.14.1.dssl1"
    original_version = "5.14.1"
    ...
    def configure(self):
        ...
        assert self.original_version == self._submodules['qtbase']['branch']
    ...
    def requirements(self):
        ...
        if self.options.with_libjpeg == "system":
            self.requires("libjpeg/9d.dssl1")
    ...
    def source(self):
        ...
        for p in self.conan_data["patches"][self.original_version]:
            tools.patch(**p)
    ...
    def build(self):
        ...
        with tools.chdir("build_folder"):
            with tools.vcvars(self.settings) if self.settings.compiler == "Visual Studio" else tools.no_op():
                ...
                with tools.environment_append(build_env):
                    configure_path = os.path.join(self.source_folder, "qt5", "configure")
                    try:
                        self.run("%s %s" % (configure_path, " ".join(args)), run_environment=True)
                    finally:
        ...
        with open('build_folder/qtbase/bin/qt.conf', 'w') as f:
            f.write('[Paths]\nPrefix = ..')

    def package(self):
        self.copy("bin/qt.conf", src="build_folder/qtbase")
