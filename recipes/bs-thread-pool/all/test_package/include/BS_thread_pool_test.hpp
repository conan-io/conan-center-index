 ...
 * pool library. This program tests all aspects of the library, but is not
 * needed in order to use the library.
 */
...
constexpr bool output_log = false;
...
// Two global synced_streams objects. One prints to std::cout, and the other to
// a file.
...
// A global random_device object to be used to seed some random number
// generators.
...
/**
 * @brief Check if a condition is met, report the result, and keep count of the
...
BS::concurrency_t count_unique_threads() {
  ...
  BS::concurrency_t unique_threads = (BS::concurrency_t)(
      std::unique(thread_IDs.begin(), thread_IDs.end()) - thread_IDs.begin());
...
void check_push_task() {
               ...
               "arguments and no return value...");
  {
    bool flag1 = false;
    bool flag2 = false;
    pool.push_task([](bool *flag1_, bool *flag2_) { *flag1_ = *flag2_ = true; },
                   &flag1, &flag2);
    pool.wait_for_tasks();
    check(flag1 && flag2);
  }
...
void check_submit() {
               ...
               "and no return value...");
  {
    bool flag1 = false;
    bool flag2 = false;
    pool.submit([](bool *flag1_, bool *flag2_) { *flag1_ = *flag2_ = true; },
                &flag1, &flag2)
        .wait();
    check(flag1 && flag2);
  }
  ...
  {
    bool flag = false;
    std::future<int> my_future = pool.submit([&flag] {
      flag = true;
      return 42;
    });
 ...
 * split over a specific number of tasks, with no return value.
 *
 * @param start The first index in the loop.
 * @param end The last index in the loop plus 1.
 * @param num_tasks The number of tasks.
 */
void check_parallelize_loop_no_return(const int64_t random_start,
                                      int64_t random_end,
                                      const BS::concurrency_t num_tasks) {
  if (random_start == random_end)
    ++random_end;
  dual_println("Verifying that a loop from ", random_start, " to ", random_end,
               " with ", num_tasks, num_tasks == 1 ? " task" : " tasks",
               " modifies all indices...");
  const size_t num_indices =
      static_cast<size_t>(std::abs(random_end - random_start));
...
/**
 * @brief Check that parallelize_loop() works for a specific number of indices
 * split over a specific number of tasks, with a return value.
 *
 * @param start The first index in the loop.
 * @param end The last index in the loop plus 1.
 * @param num_tasks The number of tasks.
 */
void check_parallelize_loop_return(const int64_t random_start,
                                   int64_t random_end,
                                   const BS::concurrency_t num_tasks) {
  if (random_start == random_end)
    ++random_end;
  dual_println("Verifying that a loop from ", random_start, " to ", random_end,
               " with ", num_tasks, num_tasks == 1 ? " task" : " tasks",
               " correctly sums all indices...");
  const std::vector<int64_t> sums_vector =
      pool.parallelize_loop(
              random_start, random_end,
  ...
  for (const int64_t &s : sums_vector)
    sum += s;
  check(sum * 2 ==
        std::abs(random_start - random_end) * (random_start + random_end - 1));
}
...
void check_parallelize_loop() {
  ...
  std::uniform_int_distribution<BS::concurrency_t> task_dist(
      1, pool.get_thread_count());
  constexpr uint64_t n = 10;
  for (uint64_t i = 0; i < n; ++i)
    check_parallelize_loop_no_return(index_dist(mt), index_dist(mt),
                                     task_dist(mt));
  for (uint64_t i = 0; i < n; ++i)
    check_parallelize_loop_return(index_dist(mt), index_dist(mt),
                                  task_dist(mt));
}
...
void check_task_monitoring() {
  BS::concurrency_t n =
      std::min<BS::concurrency_t>(std::thread::hardware_concurrency(), 4);
...
/**
 * @brief Check that parallelized vector operations work as expected by
 * calculating the sum of two randomized vectors of a specific size in two ways,
 * single-threaded and multithreaded, and comparing the results.
 */
void check_vector_of_size(const size_t vector_size,
                          const BS::concurrency_t num_tasks) {
  ...
  dual_println("Adding two vectors with ", vector_size, " elements using ",
               num_tasks, " tasks...");
  ...
  pool.parallelize_loop(
          0, vector_size,
          [&sum_multi, &vector_1, &vector_2](const size_t start,
                                             const size_t end) {
...
/**
 * @brief Check that parallelized vector operations work as expected by
 * calculating the sum of two randomized vectors in two ways, single-threaded
 * and multithreaded, and comparing the results.
 */
void check_vectors() {
  pool.reset();
  std::mt19937_64 mt(rd());
 ...
 *
 * @param num_tasks The number of tasks.
 * @param mean_sd An std::pair containing the mean as the first member and
 * standard deviation as the second member.
 */
...
void print_speedup(const std::vector<double> &timings,
                   const BS::concurrency_t try_tasks[]) {
  const std::vector<double>::const_iterator min_el =
      std::min_element(std::begin(timings), std::end(timings));
  ...
  dual_println(
      "Maximum speedup obtained by multithreading vs. single-threading: ",
      max_speedup, "x, using ", num_tasks, " tasks.");
}
 ...
 * @return An std::pair containing the mean as the first member and standard
 * deviation as the second member.
 */
std::pair<double, double>
analyze(const std::vector<std::chrono::milliseconds::rep> &timings) {
  double mean = 0;
  for (size_t i = 0; i < timings.size(); ++i)
    mean +=
        static_cast<double>(timings[i]) / static_cast<double>(timings.size());
  double variance = 0;
  for (size_t i = 0; i < timings.size(); ++i)
    variance += (static_cast<double>(timings[i]) - mean) *
                (static_cast<double>(timings[i]) - mean) /
...
/**
 * @brief Generate a seed. The std::mt19937_64 in each task will be seeded using
 * this function in order to avoid depleting the entropy of the random_device.
 *
...
void check_performance() {
  ...
  // Initialize a random distribution to randomize vectors with arbitrary
  ...
  // Define the number of tasks to try in each run of the test (0 =
  // single-threaded).
  const BS::concurrency_t try_tasks[] = {0,
                                         thread_count / 4,
                                         thread_count / 2,
                                         thread_count,
                                         thread_count * 2,
  ...
  // How many times to repeat each run of the test in order to collect reliable
  // statistics.
  constexpr size_t repeat = 20;
  dual_println("Each test will be repeated ", repeat,
               " times to collect reliable statistics.");

  // The target duration of the single-threaded test in milliseconds. The total
  // time spent on the test in the single-threaded case will be approximately
  // equal to repeat * target_ms.
  ...
  // Test how many vectors we need to generate to roughly achieve the target
  // duration.
  size_t num_vectors = 1;
  do {
    num_vectors *= 2;
    std::vector<std::vector<double>> vectors(num_vectors,
                                             std::vector<double>(vector_size));
  ...
  num_vectors = static_cast<size_t>(std::llround(
      static_cast<double>(num_vectors) * static_cast<double>(target_ms) /
      static_cast<double>(tmr.ms())));
  ...
  // Perform the test.
  dual_println("\nGenerating ", num_vectors, " random vectors with ",
               vector_size, " elements each:");
