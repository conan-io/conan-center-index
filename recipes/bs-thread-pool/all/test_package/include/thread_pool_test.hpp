...
#include "bs_thread_pool/thread_pool.hpp"
...
// Define two global synced_streams objects: one prints to std::cout and the
// other to a file.
...
/**
 * @brief Print any number of items into both std::cout and the log file,
 * syncing both independently.
 *
 * @tparam T The types of the items.
 * @param items The items to print.
 */
template <typename... T> void dual_print(const T &...items) {
...
/**
 * @brief Print any number of items into both std::cout and the log file,
 * followed by a newline character, syncing both independently.
 *
 * @tparam T The types of the items.
 * @param items The items to print.
 */
template <typename... T> void dual_println(const T &...items) {
  dual_print(items..., '\n');
}
...
std::string get_time() {
  ...
  char time_string[32];
  std::strftime(time_string, sizeof(time_string), "%Y-%m-%d_%H.%M.%S",
                std::localtime(&t));
  return std::string(time_string);
}

/**
 * @brief Check if a condition is met, report the result, and count the number
...
/**
 * @brief Store the ID of the current thread in memory. Waits for a short time
 * to ensure it does not get evaluated by more than one thread.
 *
 * @param location A pointer to the location where the thread ID should be
 * stored.
 */
...
/**
 * @brief Count the number of unique threads in the thread pool to ensure that
 * the correct number of individual threads was created. Pushes a number of
 * tasks equal to four times the thread count into the thread pool, and count
 * the number of unique thread IDs returned by the tasks.
 */
ui32 count_unique_threads() {
  ...
  ui32 unique_threads =
      (ui32)(std::unique(thread_IDs.begin(), thread_IDs.end()) -
             thread_IDs.begin());
...
void check_constructor() {
  dual_println("Checking that the thread pool reports a number of threads "
               "equal to the hardware concurrency...");
  check(pool.get_thread_count() == std::thread::hardware_concurrency());
  dual_println("Checking that the manually counted number of unique thread IDs "
               "is equal to the reported number of threads...");
...
void check_reset() {
  ...
  dual_println("Checking that after reset() the thread pool reports a number "
               "of threads equal to half the hardware concurrency...");
  check(pool.get_thread_count() == std::thread::hardware_concurrency() / 2);
  dual_println(
      "Checking that after reset() the manually counted number of unique "
      "thread IDs is equal to the reported number of threads...");
  check(pool.get_thread_count() == count_unique_threads());
  pool.reset(std::thread::hardware_concurrency());
  dual_println("Checking that after a second reset() the thread pool reports a "
               "number of threads equal to the hardware concurrency...");
  check(pool.get_thread_count() == std::thread::hardware_concurrency());
  dual_println(
      "Checking that after a second reset() the manually counted number of "
      "unique thread IDs is equal to the reported number of threads...");
  check(pool.get_thread_count() == count_unique_threads());
}
...
void check_push_task() {
  dual_println("Checking that push_task() works for a function with no "
  ...
  dual_println("Checking that push_task() works for a function with one "
               "argument and no return value...");
  {
    bool flag = false;
    pool.push_task([](bool *flag) { *flag = true; }, &flag);
    pool.wait_for_tasks();
    check(flag);
  }
  dual_println("Checking that push_task() works for a function with two "
               "arguments and no return value...");
  {
    bool flag1 = false;
    bool flag2 = false;
    pool.push_task([](bool *flag1, bool *flag2) { *flag1 = *flag2 = true; },
                   &flag1, &flag2);
    pool.wait_for_tasks();
    check(flag1 && flag2);
  }
 ...
 */
void check_submit() {
  dual_println("Checking that submit() works for a function with no arguments "
               "or return value...");
  {
    bool flag = false;
    auto my_future = pool.submit([&flag] { flag = true; });
    check(my_future.get() && flag);
  }
  dual_println("Checking that submit() works for a function with one argument "
               "and no return value...");
  {
    bool flag = false;
    auto my_future = pool.submit([](bool *flag) { *flag = true; }, &flag);
    check(my_future.get() && flag);
  }
  dual_println("Checking that submit() works for a function with two arguments "
               "and no return value...");
  {
    bool flag1 = false;
    bool flag2 = false;
    auto my_future =
        pool.submit([](bool *flag1, bool *flag2) { *flag1 = *flag2 = true; },
                    &flag1, &flag2);
...
/**
 * @brief Check that parallelize_loop() works for a specific number of indices
 * split over a specific number of tasks.
 *
...
void check_parallelize_loop(i32 start, i32 end, const ui32 &num_tasks) {
  ...
  dual_println("Verifying that a loop from ", start, " to ", end, " with ",
               num_tasks, num_tasks == 1 ? " task" : " tasks",
               " modifies all indices...");
  ...
  pool.parallelize_loop(
      start, end,
      [&flags, &offset](const i32 &start, const i32 &end) {
...
/**
 * @brief Check that parallelize_loop() works using several different random
 * values for the range of indices and number of tasks.
 */
void check_parallelize_loop() {
  std::mt19937_64 mt(rd());
  std::uniform_int_distribution<i32> index_dist(
      (i32)pool.get_thread_count() * -100, (i32)pool.get_thread_count() * 100);
...
void check_sleep_duration(const ui32 &duration) {
  dual_println("Submitting tasks with sleep_duration = ", duration,
               " microseconds...");
  pool.sleep_duration = duration;
  ui32 n = pool.get_thread_count() * 100;
  std::vector<std::atomic<bool>> flags(n);
  for (ui32 i = 0; i < n; i++)
    pool.push_task([&flags, i] { flags[i] = true; });
  pool.wait_for_tasks();
  bool all_flags = true;
  for (ui32 i = 0; i < n; i++)
    all_flags = all_flags && flags[i];
...
void check_sleep_duration() {
  ...
  dual_println("Resetting sleep_duration to the default value (", old_duration,
               " microseconds).");
...
void check_task_monitoring() {
  ...
  dual_println("After submission, should have: ", n * 3, " tasks total, ", n,
               " tasks running, ", n * 2, " tasks queued...");
  check(pool.get_tasks_total() == n * 3 && pool.get_tasks_running() == n &&
        pool.get_tasks_queued() == n * 2);
  for (ui32 i = 0; i < n; i++)
    release[i] = true;
  std::this_thread::sleep_for(std::chrono::milliseconds(300));
  dual_println("After releasing ", n, " tasks, should have: ", n * 2,
               " tasks total, ", n, " tasks running, ", n, " tasks queued...");
  for (ui32 i = n; i < n * 2; i++)
    release[i] = true;
  check(pool.get_tasks_total() == n * 2 && pool.get_tasks_running() == n &&
        pool.get_tasks_queued() == n);
  std::this_thread::sleep_for(std::chrono::milliseconds(300));
  dual_println("After releasing ", n, " more tasks, should have: ", n,
               " tasks total, ", n, " tasks running, ", 0, " tasks queued...");
  check(pool.get_tasks_total() == n && pool.get_tasks_running() == n &&
        pool.get_tasks_queued() == 0);
  for (ui32 i = n * 2; i < n * 3; i++)
    release[i] = true;
  std::this_thread::sleep_for(std::chrono::milliseconds(200));
  dual_println("After releasing the final ", n, " tasks, should have: ", 0,
               " tasks total, ", 0, " tasks running, ", 0, " tasks queued...");
  check(pool.get_tasks_total() == 0 && pool.get_tasks_running() == 0 &&
        pool.get_tasks_queued() == 0);
  dual_println("Resetting pool to ", std::thread::hardware_concurrency(),
               " threads.");
  pool.reset(std::thread::hardware_concurrency());
}
...
void check_pausing() {
  ...
  dual_println("Immediately after submission, should have: ", n * 3,
               " tasks total, ", 0, " tasks running, ", n * 3,
               " tasks queued...");
  check(pool.get_tasks_total() == n * 3 && pool.get_tasks_running() == 0 &&
        pool.get_tasks_queued() == n * 3);
  std::this_thread::sleep_for(std::chrono::milliseconds(300));
  dual_println("300ms later, should still have: ", n * 3, " tasks total, ", 0,
               " tasks running, ", n * 3, " tasks queued...");
  check(pool.get_tasks_total() == n * 3 && pool.get_tasks_running() == 0 &&
        pool.get_tasks_queued() == n * 3);
  dual_println("Unpausing pool.");
  pool.paused = false;
  std::this_thread::sleep_for(std::chrono::milliseconds(300));
  dual_println("300ms later, should have: ", n * 2, " tasks total, ", n,
               " tasks running, ", n, " tasks queued...");
  check(pool.get_tasks_total() == n * 2 && pool.get_tasks_running() == n &&
        pool.get_tasks_queued() == n);
  dual_println(
  ...
  pool.paused = true;
  pool.wait_for_tasks();
  dual_println("After waiting, should have: ", n, " tasks total, ", 0,
               " tasks running, ", n, " tasks queued...");
  check(pool.get_tasks_total() == n && pool.get_tasks_running() == 0 &&
        pool.get_tasks_queued() == n);
  std::this_thread::sleep_for(std::chrono::milliseconds(200));
  dual_println("200ms later, should still have: ", n, " tasks total, ", 0,
               " tasks running, ", n, " tasks queued...");
  check(pool.get_tasks_total() == n && pool.get_tasks_running() == 0 &&
        pool.get_tasks_queued() == n);
  dual_println(
  ...
  pool.paused = false;
  pool.wait_for_tasks();
  dual_println("After waiting, should have: ", 0, " tasks total, ", 0,
               " tasks running, ", 0, " tasks queued...");
  check(pool.get_tasks_total() == 0 && pool.get_tasks_running() == 0 &&
        pool.get_tasks_queued() == 0);
  dual_println("Resetting pool to ", std::thread::hardware_concurrency(),
               " threads.");
...
void check_exceptions() {
  bool caught = false;
  auto my_future =
      pool.submit([] { throw std::runtime_error("Exception thrown!"); });
  try {
    my_future.get();
...
/**
 * @brief A lightweight matrix class template for performance testing purposes.
 * Not for general use; only contains the bare minimum functionality needed for
 * the test. Based on https://github.com/bshoshany/multithreaded-matrix
 *
...
public:
  ...
  /**
   * @brief Construct a new matrix by copying the elements of an existing
   * matrix.
   *
   ...
   * @return A reference to the element.
   */
  inline T &operator[](const ui64 &i) { return elements[i]; }
   ...
   * @return The value of the element.
   */
  inline T operator[](const ui64 &i) const { return elements[i]; }
   ...
   *
   * @param num_tasks The number of parallel tasks to use. If set to 0, no
  ...
  matrix<T> transpose(const ui32 &num_tasks) const {
    ...
    if (num_tasks == 0) {
      ...
      pool.parallelize_loop(
          0, out.rows,
          [this, &out](const ui64 &start, const ui64 &end) {
   ...
   */
  friend matrix<T> add_matrices(const matrix<T> &a, const matrix<T> &b,
                                const ui32 &num_tasks) {
    ...
    if (num_tasks == 0)
      ...
      pool.parallelize_loop(
          0, a.rows * a.cols,
          [&a, &b, &c](const ui64 &start, const ui64 &end) {
            for (ui64 i = start; i < end; i++)
              c[i] = a[i] + b[i];
          },
          num_tasks);
   ...
   */
  friend matrix<T> multiply_matrices(const matrix<T> &a, const matrix<T> &b,
                                     const ui32 &num_tasks) {
    ...
    if (num_tasks == 0) {
      ...
      pool.parallelize_loop(
          0, a.rows,
          [&a, &b, &c, &a_cols = a.cols, &b_cols = b.cols](const ui64 &start,
                                                           const ui64 &end) {
            for (ui64 i = start; i < end; i++)
...
private:
  ...
  /**
   * @brief A pointer to an array storing the elements of the matrix in
   * flattened 1-dimensional form.
   */
  T *elements = nullptr;

  /**
   * @brief A smart pointer to manage the memory allocated for the matrix
   * elements.
   */
 ...
 *
 * @tparam T The type to use for the matrix elements.
 * @tparam D The distribution to use, e.g.
 * std::uniform_real_distribution<double>.
 */
template <typename T, typename D> class random_matrix_generator {
public:
  // ============
  // Constructors
  // ============
   ...
   *
   * @tparam P The types of the parameters to pass to the constructor of the
   * distribution.
   * @param params The parameters to pass to the constructor of the
   * distribution. The number of parameters and their types depends on the
   * particular distribution being used.
   */
  template <typename... P>
  ...
  matrix<T> generate_matrix(const ui64 &rows, const ui64 &cols,
    ...
    if (num_tasks == 0) {
      ...
      pool.parallelize_loop(
          0, rows * cols,
          [this, &m](const ui64 &start, const ui64 &end) {
            std::mt19937_64 mt(generate_seed());
            for (ui64 i = start; i < end; i++)
              m[i] = dist(mt);
          },
...
private:
  ...
  /**
   * @brief Generate a seed. The std::mt19937_64 in each block will be seeded
   * using this function in order to avoid depleting the entropy of the
   * random_device.
   *
  ...
  /**
   * @brief The random device to be used for seeding the pseudo-random number
   * generators.
   */
  std::random_device rd;
};
...
/**
 * @brief Check the matrix class template by comparing the results of adding,
 * multiplying, and transposing matrices calculated in two ways: single-threaded
 * and multithreaded.
 */
void check_matrix() {
  // Initialize a random_matrix_generator object to generates matrices with
  // integers uniformly distributed between -1000 and 1000.
  random_matrix_generator<i64, std::uniform_int_distribution<i64>> rnd(-1000,
                                                                       1000);
  ...
  dual_println("Using matrices of size ", rows, "x", cols, " with a total of ",
               total_size, " elements.");
 ...
 *
 * @param num_tasks The number of tasks.
 * @param mean_sd std::pair containing the mean as the first member and standard
 * deviation as the second member.
 */
void print_timing(const ui32 &num_tasks,
                  const std::pair<double, double> &mean_sd) {
  if (num_tasks == 1)
    dual_print("With   1  task");
  else
    dual_print("With ", std::setw(3), num_tasks, " tasks");
  dual_println(", mean execution time was ", std::setw(6), mean_sd.first,
               " ms with standard deviation ", std::setw(4), mean_sd.second,
               " ms.");
}
 ...
 *
 * @param timings A vector of the timings corresponding to different numbers of
 * tasks.
 * @return The maximum speedup obtained.
 */
double print_speedup(const std::vector<double> &timings) {
  const auto [min_time, max_time] =
      std::minmax_element(std::begin(timings), std::end(timings));
 ...
 * @return std::pair containing the mean as the first member and standard
 * deviation as the second member.
 */
std::pair<double, double> analyze(const std::vector<i64> &timings) {
  ...
  double variance = 0;
  for (size_t i = 0; i < timings.size(); i++)
    variance += ((double)timings[i] - mean) * ((double)timings[i] - mean) /
                (double)timings.size();
...
void check_performance() {
  ...
  // Initialize a random_matrix_generator object to generates matrices with real
  // (floating-point) numbers uniformly distributed between -1000 and 1000.
  random_matrix_generator<double, std::uniform_real_distribution<double>> rnd(
      -1000, 1000);

  // Initialize a timer object to measure the execution time of various
  // operations.
  timer tmr;

  // If the CPU has more than 8 threads, we leave 2 threads for the rest of the
  // operating system. Otherwise, performance may suffer.
  const ui32 thread_count = pool.get_thread_count() <= 8
                                ? pool.get_thread_count()
                                : pool.get_thread_count() - 2;
  dual_println("Using ", thread_count, " out of ", pool.get_thread_count(),
               " threads.");
  ...
  // Generate two random test matrices to be used for benchmarking addition,
  // transposition, and random matrix generation.
  matrix<double> A = rnd.generate_matrix(rows, cols, thread_count);
  matrix<double> B = rnd.generate_matrix(rows, cols, thread_count);

  // Generate two random test matrices to be used for benchmarking
  // multiplication. Since matrix multiplication is O(n^3), we reduce the size
  // of the test matrices so that this operation completes within a reasonable
  // time.
  constexpr ui64 mult_factor = 8;
  matrix<double> X =
      rnd.generate_matrix(rows / mult_factor, cols / mult_factor, thread_count);
  matrix<double> Y =
      rnd.generate_matrix(cols / mult_factor, rows / mult_factor, thread_count);
  ...
  if (optimal_sleep == 0)
    dual_println("\nResult: Using std::this_thread::yield() instead of "
                 "std::this_thread::sleep_for() is optimal.");
  else
    dual_println("\nResult: The optimal sleep duration is ", optimal_sleep,
                 " microseconds.");
  ...
  dual_println("\nAdding two ", rows, "x", cols, " matrices ", repeat,
               " times:");
  ...
  dual_println("\nTransposing one ", rows, "x", cols, " matrix ", repeat,
               " times:");
  for (ui32 n : try_tasks) {
    for (ui32 i = 0; i < repeat; i++) {
      tmr.start();
  ...
  dual_println("\nMultiplying two ", rows / mult_factor, "x",
               cols / mult_factor, " matrices ", repeat, " times:");
  for (ui32 n : try_tasks) {
    for (ui32 i = 0; i < repeat; i++) {
      tmr.start();
  ...
  dual_println("\nGenerating random ", rows, "x", cols, " matrix ", repeat,
               " times:");
  for (ui32 n : try_tasks) {
    for (ui32 i = 0; i < repeat; i++) {
      tmr.start();
  ...
  dual_println("\nOverall, multithreading provided speedups of up to ",
               max_speedup, "x.");
}
...
int main() {
  ...
  dual_println(
      "Important: Please do not run any other applications, especially "
      "multithreaded applications, in parallel with this test!");
  ...
  if (tests_failed == 0) {
    print_header("SUCCESS: Passed all " + std::to_string(tests_succeeded) +
                     " checks!",
                 '+');
    ...
    print_header("FAILURE: Passed " + std::to_string(tests_succeeded) +
                     " checks, but failed " + std::to_string(tests_failed) +
                     "!",
                 '+');
    dual_println("\nPlease submit a bug report at "
                 "https://github.com/bshoshany/thread-pool/issues including "
                 "the exact specifications of your system (OS, CPU, compiler, "
                 "etc.) and the generated log file.");
  }
