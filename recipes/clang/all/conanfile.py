...
def components_from_dotfile(dotfile, exclude_labels):
    """
    ...
    def node_labels(dot):
        ...
        for row in dot:
            match_label = re.match(
                r'''^"(node[0-9]+)"\s*\[\s*label\s*=\s*"(.+)"''', row.strip()
            )
            if match_label:
    ...
    def component_and_dependency(dot):
        ...
        def is_package_label(label):
            ...
            return label.startswith("clang") and not label.endswith("-resource-headers") and label not in exclude_labels

        labels = {k: v for k, v in node_labels(dot)}
        for row in dot:
            match_dep = re.match(
                r'''"(node[0-9]+)"\s*->\s*"(node[0-9]+)"''', row.strip()
            )
            if match_dep:
    ...
    def sanitize_dependency(dependency_name):
        ...
        """
        system_libs = ["pthread", "dl", "m", "version"]
...
class ClangConan(ConanFile):
    ...
    def build(self):
        ...
        exclude_patterns = ["CONAN_LIB*", "llvm-core*"]
        ...
        save(
            self,
            PurePosixPath(self.build_folder) / "CMakeGraphVizOptions.cmake",
            graphviz_options,
        )
    ...
    def _build_module_file_rel_path(self):
        return (
            self._cmake_build_folder_rel_path
    ...
    def _build_info_file(self):
        return (
            ...
            / "conan_clang_build_info.json"
        )

    def _write_build_info(self):
        build_info = {
            "components": components_from_dotfile(
                load(self, self._graphviz_file.as_posix()),
                exclude_labels=[] if self.options.shared else ["clang-cpp"],
            )
        }
    ...
    def package_info(self):
        ...
        for component, data in components.items():
            ...
            self.cpp_info.components[component].libs = [
                _lib_name_from_component(component)
            ]
