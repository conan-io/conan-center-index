        ...
        if not tools.cross_building(self):
            ...
            if not os.path.isfile(self._test_package_assembly_source):
                ...
                if self._target_os == "Windows":
                    ...
                    dlltool_args = [
                        dlltool, "--input-def", f"{self.source_folder}/Windows-kernel32.def", "--output-lib", f"{self.build_folder}/lib/libkernel32.a"]
                    self.run(" ".join(dlltool_args))

                assembler_args = [gas, self._test_package_assembly_source, "-o", f"{self.build_folder}/object.o"]
                linker_args = [ld, f"{self.build_folder}/object.o", "-o",
                               f"{self.build_folder}/bin/test_package{extension}"] + self._test_linker_args
