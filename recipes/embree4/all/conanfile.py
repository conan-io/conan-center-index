...
class Embree4Conan(ConanFile):
    ...
    def validate(self):
        if not (
            self._has_sse_avx or (self._embree_has_neon_support and self._has_neon)
        ):
            raise ConanInvalidConfiguration(
                f"{self.ref} doesn't support {self.settings.arch}"
            )
    ...
    def generate(self):
        ...
        tc.variables["EMBREE_TASKING_SYSTEM"] = (
            "TBB" if self.options.with_tbb else "INTERNAL"
        )
