...
class TestPackageConan(ConanFile):
    ...
    def _with_legacy(self):
        return (not self.options["quictls-openssl"].no_legacy and
            ((not self.options["quictls-openssl"].no_md4) or
              (not self.options["quictls-openssl"].no_rmd160)))

    def build(self):
        cmake = CMake(self)
        cmake.definitions["OPENSSL_WITH_ZLIB"] = not self.options["quictls-openssl"].no_zlib
        cmake.definitions["OPENSSL_WITH_LEGACY"] = self._with_legacy()
        cmake.definitions["OPENSSL_WITH_MD4"] = not self.options["quictls-openssl"].no_md4
        cmake.definitions["OPENSSL_WITH_RIPEMD160"] = not self.options["quictls-openssl"].no_rmd160
