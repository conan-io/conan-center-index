...
Note about `build_type`.- We retain the `build_type` setting to make it possible for the users to _debug_ these installer packages. We could remove this settings and it will be possible to compile these packages in _debug_ mode, but if we remove it from the packageID, the compiled package would override the existing _release_ binary, and it'd be quite inconvenient for the users to compile the binary every time they need to switch from _debug_ to _release_.
