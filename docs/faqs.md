...
## Why is `tools.check_min_cppstd call` not enough?

Very often C++ projects require a minimum standard version, such as 14 or 17, in order to compiler. Conan offers tools which enable checking this setting is enabled above this a certain version otherwise it uses the compiler's default.
...
3. Conan-Center-Index to does manage the `cppstd` setting in the binaries it generates
...
As a result, all calls to `tools.check_min_cppstd` must be garded by a check for the setting and the only way to ensure the C++ standard is to check the compiler's version to know if it offer sufficient support. An example of this can be found [here](https://github.com/conan-io/conan/issues/8002).
