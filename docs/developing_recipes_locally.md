...
We recommend working from the `recipes/project` folder itself. You can learn about the [recipe file structure](adding_packages/how_to_add_packages.md#recipe-files-structure) to understand the folder and files located there.

> **Note**: You can only change one recipe per pull request, and working from the [_recipe folder_](adding_packages/how_to_add_packages.md#the-recipe-folder-conanfilepy) will help prevent making a few mistakes. The default for this folder is `all`, follow the link above to learn more.
...
build script that are tailored to their specific use cases. It's not uncommon to [patch build scripts](add_packages/policy_patching.md#policy-about-patching) but make sure to read the [patch policy](adding_packages/policy_patching.md). You are encouraged to submit pull requests upstream.
